...
TEST(TransferFunction, CanCreateTransferFunctionFromCoefficients)
{
    ...
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_Poly_AllocPersistent(&ctx, &num, n_val, 2));
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_Poly_AllocPersistent(&ctx, &dem, d_val, 3));

    // Assert that the TF construction succeeds
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_TF_FromPoly(&ctx, &tf, &num, &dem));
...
TEST(TransferFunction, MultiplyTwoTransferFunctions)
{
    ...
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_TF_Multiply(&ctx, &G_sys, &G1, &G2));
...
TEST(TransferFunction, UnityClosedLoopReduction)
{
    ...
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_TF_ClosedLoop(&ctx, &T, &G, 1.0f, TF_FEEDBACK_NEGATIVE));
...
{
    ControlTransferFunction empty = CONTROL_EMPTY_TF;

    TEST_ASSERT_EQUAL(CONTROL_ERROR_INVALID_ARGUMENT, Control_TF_Validate(&ctx, &empty));
    TEST_ASSERT_EQUAL(CONTROL_ERROR_INVALID_ARGUMENT, last_error_code);
}
...
TEST(TransferFunction, MultiplyInvalidTransferFunctionErrorsInvalidArgument)
{
    ...
    ControlTransferFunction G2     = CONTROL_EMPTY_TF;
    ControlTransferFunction result = {0};

    ControlResult status = Control_TF_Multiply(&ctx, &result, &G1, &G2);

    TEST_ASSERT_EQUAL(CONTROL_ERROR_INVALID_ARGUMENT, Control_TF_Validate(&ctx, &result));
    TEST_ASSERT_EQUAL(CONTROL_ERROR_INVALID_ARGUMENT, status);
    TEST_ASSERT_EQUAL(CONTROL_ERROR_INVALID_ARGUMENT, last_error_code);
}
...
TEST(TransferFunction, MultiplyOutOfMemorySafelyAbortsAndThrows)
{
    ...
    TEST_ASSERT_EQUAL(CONTROL_ERROR_OUT_OF_MEMORY, status);
    TEST_ASSERT_EQUAL(CONTROL_ERROR_OUT_OF_MEMORY, last_error_code);
    TEST_ASSERT_EQUAL(CONTROL_ERROR_INVALID_ARGUMENT, Control_TF_Validate(&ctx, &result));
    TEST_ASSERT_EQUAL(CONTROL_ERROR_INVALID_ARGUMENT, last_error_code);
}
...
TEST(TransferFunction, CanDeepCloneTransferFunction)
{
    ...
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_TF_Persist(&ctx, &cloned_tf, &G1));

    TEST_ASSERT_EQUAL(CONTROL_OK, Control_TF_Validate(&ctx, &cloned_tf));
