...
TEST(StateSpace, CanConvertTransferFunction)
{
    ...
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_Poly_AllocPersistent(&ctx, &num, n, 1));
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_Poly_AllocPersistent(&ctx, &dem, d, 3));
    TEST_ASSERT_EQUAL(CONTROL_OK, Control_TF_FromPoly(&ctx, &tf, &num, &dem));

    TEST_ASSERT_EQUAL(CONTROL_OK, Control_StateSpace_FromTF(&ctx, &sys, &tf));
